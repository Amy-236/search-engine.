...
import com.example.searchengine.index.IndexStorage;
import com.example.searchengine.index.InvertedIndex;
import com.example.searchengine.mapper.RepositoryMapper;
import com.example.searchengine.model.SearchResult;
...
public class IndexService {
    ...
    public void buildIndex() {
        ...
        while (true) {
            // 使用MyBatis-Plus的分页查询
            Page<RepositoryEntity> page = new Page<>(current, pageSize);
            Page<RepositoryEntity> result = repositoryMapper.selectPage(page, 
                new LambdaQueryWrapper<RepositoryEntity>()
                    .isNotNull(RepositoryEntity::getReadme));
            
            List<RepositoryEntity> repositories = result.getRecords();
     ...
     * @return 搜索结果列表
     */
    public List<RepositoryEntity> search(String query) {
        try {
            // 1. 获取倒排索引搜索结果
            ...
            // 4. 按顺序查询仓库信息
            List<RepositoryEntity> repositoryEntityList = repositoryMapper.selectByIdsInOrder(resultIds);

            // 5. 对readme字段进行截取
            List<RepositoryEntity> result = new ArrayList<>();
            for (RepositoryEntity repository :repositoryEntityList) {
